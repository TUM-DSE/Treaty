...
namespace erpc {
	...
	class HugeAlloc {
		public:
			...
			std::atomic<uint64_t> allocated_mem = {0}; // not thread_safe
			std::atomic<uint64_t> freed_mem = {0}; // not thread_safe
			std::atomic<uint64_t> user_allocated_mem = {0}; // not thread_safe
			...
			inline void free_buf(Buffer buffer) {
				...
				freed_mem.fetch_add(buffer.class_size);
			}
		...
		private:
			...
			// @dimitra: this is added for statistic purposes to track the nb of syscalls for mem-allocation
			size_t nb_mmap_calls;
			...
			inline Buffer alloc_from_class(size_t size_class) {
				...
				stats.user_alloc_tot += buffer.class_size;
        user_allocated_mem.fetch_add(buffer.class_size);
				return buffer;
			}
