		...
		struct mapped_pci_resource *uio_res, int map_idx)
{
	...
	//fd = scone_proc_open(devname, O_RDWR);
	if (fd < 0) {
		RTE_LOG(ERR, EAL, "Cannot open %s: %s\n",
				devname, strerror(errno));
		goto error;
	...
	fprintf(stdout, "%s():\n -- dev->mem_resource[%d].phys_addr = %p\n -- dev->mem_resource[%d].addr = %p\n", __func__, res_idx, dev->mem_resource[res_idx].phys_addr, res_idx, mapaddr);
	return 0;
