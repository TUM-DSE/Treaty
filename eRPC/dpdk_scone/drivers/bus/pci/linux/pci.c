...
pci_get_kernel_driver_by_path(const char *filename, char *dri_name)
{
	...
	if (count < 0) {
		/* SCONE implementation for readlink() returns -1 
		if (strstr(filename, "03") != NULL) {
			count = scone_proc_readlink(filename, path, PATH_MAX);
		}*/
		if (count < 0)
			return 1;
	}
...
rte_pci_map_device(struct rte_pci_device *dev)
{
	...
	switch (dev->kdrv) {
		...
		default:
			RTE_LOG(INFO, EAL,
					"  %s(): Not managed by a supported kernel driver, skipped\n", __func__);
			ret = 1;
			break;
	}
