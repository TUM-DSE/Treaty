...
rte_eal_hugepage_init(void)
{
	...
	for (i = 0; i < internal_config.num_hugepage_sizes; i ++){
		...
		for (j = 0; j < hpi->num_pages[0]; j++) {
			...
			addr = scone_kernel_mmap(NULL, hpi->hugepage_sz, PROT_READ|PROT_WRITE,
...
rte_eal_hugepage_attach(void)
{
	...
	hpi = scone_kernel_mmap(NULL, sizeof(struct hugepage_info), PROT_READ, MAP_PRIVATE,
			fd_hugepage_info, 0);
	if (hpi == MAP_FAILED) {
	...
	for (i = 0; i < hpi->num_pages[0]; i++) {
		...
		addr = scone_kernel_mmap(seg->addr, hpi->hugepage_sz, PROT_READ|PROT_WRITE,
