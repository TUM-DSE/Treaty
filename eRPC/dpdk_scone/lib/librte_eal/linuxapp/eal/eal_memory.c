...
static inline int digitsNum(int n) {
	int count = 0;
	while (n != 0) {
		n /= 10;
		++count;
...
__rte_mem_virt2phy(const void * virtaddr) {
	...
	snprintf(buff, digitsNum(getpid()) + 15, "/proc/%d/pagemap", getpid());
	fd = open("/proc/self/pagemap", O_RDONLY);
	//fd = scone_proc_open(buff, O_RDONLY);
	if (fd < 0) {
		RTE_LOG(ERR, EAL, "%s(): cannot open /proc/self/pagemap: %s (file_descriptor: %d)\n",
	...
	//if (scone_proc_lseek(fd, offset, SEEK_SET) == (off_t) -1) {
	...
	close(fd);
	//retval = scone_proc_read(fd, &page, PFN_MASK_SIZE);
	//scone_proc_close(fd);
	...
	if ((page & 0x7fffffffffffffULL) == 0) {
		RTE_LOG(ERR, EAL, "page (%" PRIu64 ") == RTE_BAD_IOVA\n", page);
		return RTE_BAD_IOVA;
	}
...
test_phys_addrs_available(void)
{
	...
	if (physaddr == RTE_BAD_PHYS_ADDR) {
		RTE_LOG(ERR, EAL, "physaddr (%" PRIu64 ") == RTE_BAD_PHYS_ADDR\n", physaddr);
		phys_addrs_available = false;
 ...
 * hugetlbfs, then scone_kernel_mmap() hugepage_sz data in it. If orig is set, the
		...
		uint64_t *essential_memory __rte_unused, int orig)
{
	...
	for (i = 0; i < hpi->num_pages[0]; i++) {
		...
		if (orig) {
			 ...
			 * enforced at fault time instead of scone_kernel_mmap(), even
...
find_numasocket(struct hugepage_file *hugepg_tbl, struct hugepage_info *hpi)
{
	...
	if (f == NULL) {
		/*
		char buff[100];
		// snprintf(buff, digitsNum(getpid()) + 17, "/proc/%d/numa_maps", getpid());
		
		scone_proc_copy_procFile_to_user_space(buff);
		fprintf(stdout, "RRRRRREADY to go!!\n");
		// f = fopen("/home/dimitra/workspace/src/dpdk/proc/numa_maps", "r");
		*/
	...
	while (fgets(buf, sizeof(buf), f) != NULL) {
		...
		if (virt_addr == 0 || end == buf) {
			RTE_LOG(ERR, EAL, "%s(): error in numa_maps parsing (1)\n", __func__);
			goto error;
		}
		...
		if (nodestr == NULL) {
			RTE_LOG(ERR, EAL, "%s(): error in numa_maps parsing (2)\n", __func__);
			goto error;
		}
		nodestr += 2;
		end = strstr(nodestr, "=");
		if (end == NULL) {
			RTE_LOG(ERR, EAL, "%s(): error in numa_maps parsing (3)\n", __func__);
			goto error;
		}
		end[0] = '\0';
		end = NULL;
		...
		if ((nodestr[0] == '\0') || (end == NULL) || (*end != '\0')) {
			RTE_LOG(ERR, EAL, "%s(): error in numa_maps parsing (4)\n", __func__);
			goto error;
		}
...
rte_eal_hugepage_init(void)
{
	...
	if (internal_config.no_hugetlbfs) {
		...
		if (addr == MAP_FAILED) {
			RTE_LOG(ERR, EAL, "%s: scone_kernel_mmap() failed: %s\n", __func__,
					strerror(errno));
			return -1;
		}
		if (rte_eal_iova_mode() == RTE_IOVA_VA)
