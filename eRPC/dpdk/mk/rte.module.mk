...
$(MODULE).ko: $(SRCS_LINKS)
	@echo BUILD-MODULE EDWWWWWWWWWWWWWW $(MODULE).ko
	@if [ ! -f $(notdir Makefile) ]; then ln -nfs $(SRCDIR)/Makefile . ; fi
	@echo $(MAKE) -C $(RTE_KERNELDIR) M=$(CURDIR) O=$(RTE_KERNELDIR) CC="$(KERNELCC)" CROSS_COMPILE=$(CROSS) V=$(if $V,1,0)

	#@$(MAKE) -C $(RTE_KERNELDIR) M=$(CURDIR) O=$(RTE_KERNELDIR) \

	@$(MAKE) -C $(RTE_KERNELDIR) M=$(CURDIR) O=$(RTE_KERNELDIR) \
		CC="$(KERNELCC)" CROSS_COMPILE=$(CROSS) V=$(if $V,1,0)
	@echo BUILD-MODULE $(MODULE).ko
