...
// std::string kDBPath = "/tmp/rocksdb_rose";
...
// parse all these through GFLAGS
/*
   ...
   constexpr int current_node_id = 2;
...
DEFINE_int32(current_node_id, 2, " ");
...
DEFINE_string(kDBPath, "/tmp/rocksdb_rose", " ");
...
void coordinator(int* ptr, erpc::Nexus* nexus, AppContext* context) {
  ...
  int session_num_martha = context->rpc->create_session(martha_uri, FLAGS_cpu_cores - *ptr - 1);


  std::cout << __PRETTY_FUNCTION__ << " " << std::this_thread::get_id() << "\n";

  printf("2pc-eRPC: Coordinator Creating sessions to %s and %s, Remote RPC IDs = %d (session_num : %d) and %d (session_num: %d).\n", donna_uri.c_str(), martha_uri.c_str(), (FLAGS_cpu_cores - 1 - *ptr), session_num_donna, (FLAGS_cpu_cores - *ptr - 1), session_num_martha);

  while (!context->rpc->is_connected(session_num_martha)) context->rpc->run_event_loop_once();
  printf("Connection succeeded with martha\n");
  while (!context->rpc->is_connected(session_num_donna)) context->rpc->run_event_loop_once();
  printf("Connection succeeded with donna\n");
